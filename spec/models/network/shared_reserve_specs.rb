RSpec.shared_examples 'a network reserve' do
  context 'adding 5 in frame 0' do
    ...
    context 'then adding 2.5 more in frame 0' do
      before { reserve.add(0, 2.5) }

      it 'has 7.5 stored in frame 0' do
        expect(reserve.at(0)).to eq(7.5)
      end

      it 'carries 7.5 over to the start of frame 1' do
        expect(reserve.at(1)).to eq(7.5)
      end

      it 'has infinite unfilled volume' do
        expect(reserve.unfilled_at(0)).to eq(Float::INFINITY)
      end
    end

    context 'adding 2.5 in frame 1' do
      before { reserve.add(1, 2.5) }
...
end # shared_examples 'a network reserve'
