...
RSpec.describe LoadProfile, type: :model do
  describe 'when saved' do
    ...
    context 'the original file' do
      let(:curve) do
        Merit::Curve.load_file(load_profile.load_curves.first.curve.path(:original))
      end

      it 'is saved' do
        expect(File.file?(load_profile.load_curves.first.curve.path)).to be
      end
    ...
    context 'the capacity-scaled profile' do
      let(:curve) do
        Merit::Curve.load_file(load_profile.load_curves.first.curve.path(:capacity_scaled))
      end

      it 'is saved' do
        expect(File.file?(load_profile.load_curves.first.curve.path(:capacity_scaled))).to be
      end
    ...
    context 'the demand-scaled profile' do
      let(:curve) do
        Merit::Curve.load_file(load_profile.load_curves.first.curve.path(:demand_scaled))
      end

      it 'is saved' do
        expect(File.file?(load_profile.load_curves.first.curve.path(:demand_scaled))).to be
      end
