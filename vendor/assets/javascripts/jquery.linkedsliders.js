...
/**
 * This version has been modified to work with the Boostrap sliders.
 *
 * slider('value')       => slider('value')
 * slider('option', ...) => slider('getAttribute', ...)
 *
 * references to jQuery UI were removed, and the change event is set to trigger
 * the Boostrap slider onChange and onSlide events.
 *
 * ~ @antw (26 Jul 2016)
 */
...
$.extend(LinkedSliders.prototype, {
	...
	_optionPlugin: function(target, options, value) {
		...
		target.unbind('.' + this.propertyName).
			bind('change.' + this.propertyName + ',slide.' + this.propertyName, function(event) {
				if (!plugin._linking) {
					plugin._linking = true; // Prevent recursion
					plugin._linkSliders(target, event);
					plugin._linking = false;
				}
			});
		plugin._linkSliders(target, null); // Initial synch
	},
	...
	_linkSliders: function(target, event, ui) {
		...
		var curIndex = linked.index($((event || {}).target));
		var remTotal = inst.options.total;
		linked.each(function(i) {
			remTotal -= $(this).slider('getValue');
		});
		...
		for (var i = 0; i < 2 * linked.length; i++) {
			if (index != curIndex) {
				...
				slider.slider('setValue', newVal, true, true);
				remTotal -= (newVal - val);
				if (remTotal == 0) {
					break;
				}
