...
maxPower = d3.max(POWERS, (p) -> p.multiple)
minPower = d3.min(POWERS, (p) -> p.multiple)
...
class @Quantity
  ...
  smartScale: ->
    value    = @toBase().value
    multiple = Math.pow(1000, @power_of_thousand(value))
    ...
    power = if multiple < minPower.multiple
      ...
      POWERS.find((x) -> x.multiple is multiple)
