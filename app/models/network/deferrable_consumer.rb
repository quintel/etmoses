module Network
  ...
  class DeferrableConsumer < Technology
    extend Disableable
    ...
    def mandatory_consumption_at(frame)
      ...
      defer = @deferreds.detect do |deferred|
        # Deferreds are ordered by their mandatory_at value; if deferred occurs
        # after the given frame, we can return early.
    ...
    # Public: All non-deferred loads, and deferred load not yet at their expiry
    # date, are considered conditional.
    #
    # Returns a numeric.
    ...
    def store(frame, amount)
      ...
      if @deferreds.any?
        # If there are any deferred loads waiting to be satisfied; reduce their
        # loads first, before satisfying the demands of the current frame.
